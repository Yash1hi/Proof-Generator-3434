...
trait Expr {
  ...
  def uniqueVariables: Set[Variable] = this match {
    ...
    case Equivalent(e1, e2) => e1.uniqueVariables ++ e2.uniqueVariables
  }
  ...
  def findMatches(e: Expr, varMap: Map[Variable, Expr] = Map[Variable, Expr]()): Map[Variable, Expr] = {
    ...
    (this, e) match {
      ...
      case (Equivalent(e1, e2), Equivalent(e3, e4)) => {
        (e1, e3) match {
          case (e1: Variable, e3) => {
            if (newMap.keySet.contains(e1)) {
              if (newMap.get(e1).get != e3) {
  ...
  def replaceVars(replacements: Map[Variable, Expr]): Expr = this match {
    case v: Variable => replacements.getOrElse(v, v)
    ...
    case Equivalent(e1, e2) => Equivalent(e1.replaceVars(replacements), e2.replaceVars(replacements))
  }
  ...
  override def equals(obj: Any): Boolean = obj match {
    case that: Expr =>
      (this, that) match {
        ...
        case (Equivalent(e1, e2), Equivalent(e3, e4)) => e1 == e3 && e2 == e4
        case _ => false
      }
    case _ => false
  }
  ...
  override def hashCode: Int = this match {
    ...
    case Equivalent(e1, e2) => 103 * e1.hashCode + 17 * e2.hashCode
  }
}
...
case class Equivalent(e1: Expr, e2: Expr) extends Expr
